...
리전은 사용자가 지정한 `ap-southeast-2`를 기준으로 합니다. 콘솔에서 직접 설정할 항목은 [AWS 콘솔 설정 체크리스트](AWS_CONSOLE_CHECKLIST_ko.md)를 따릅니다.
...
- [AWS 콘솔 설정 체크리스트](AWS_CONSOLE_CHECKLIST_ko.md)
...
프로그램은 반복 실행 중에도 이 값을 다시 읽습니다. `true`이면 신규 매수 실행을 막고, live 모드에서는 SSM 읽기 실패 자체도 시작 차단 사유입니다.
...
1. GitHub Actions에서 PRISM-INSIGHT 원본과 optimization 모듈 병합
2. 페이퍼트레이딩 모드 실행
3. AWS EC2에 동일 환경 구성
