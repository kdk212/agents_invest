(() => {
  ...
  function card(x) {
    ...
    const score = Number.isFinite(rawScore) ? rawScore.toFixed(2) : num(fallbackScore, 2);
  ...
  async function render() {
    ...
    function show(index) {
      ...
      if (meta) meta.textContent = `${item.metadata?.signal_at || item.date} 기준 · ${item.metadata?.buy_at || item.date + " 시초가"} 진입 · AI WIN 원점수는 날짜별 종목 고유 점수`;
